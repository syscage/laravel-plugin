...
final class PluginRecordRepositoryTest extends TestCase
{
    ...
    public function test_it_queries_through_a_host_app_configured_model_subclass(): void
    {
        config(['plugin.model' => CustomPluginRecordForTesting::class]);

        $repository = new PluginRecordRepository();
        $record = $repository->create($this->attributes());

        $this->assertInstanceOf(CustomPluginRecordForTesting::class, $record);
        $this->assertInstanceOf(CustomPluginRecordForTesting::class, $repository->findByAlias('demo-plugin'));
    }
}

final class CustomPluginRecordForTesting extends PluginRecord
{
}
